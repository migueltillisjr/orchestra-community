...
1. **Read the evaluation first.** Open `ai_vid/01_stage` and identify every criterion that is NOT Competent.
2. **Read the current report.** Open `ai_vid/01_stage`.
3. **Read the stage instructions.** Open `ai_vid/01_stage` for the full revision procedure.
...
6. **Write the result to disk.** Use the `write` tool to save the complete revised report to `ai_vid/01_stage`.

## Output

Use the `write` tool to create `ai_vid/01_stage` on disk. Do NOT just display in chat. The file must be a complete, standalone report.
