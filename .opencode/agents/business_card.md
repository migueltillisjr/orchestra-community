...
Edit the source section files and `CARD_DATA.json`, not the generated assembled HTML, then run `business_card/.biz_card/bin/python business_card/01_stage/references/scripts/build_template.py`. Every build removes and regenerates `qr-code.jpg` by explicitly passing `CARD_DATA.json` to `scripts/generate_qr_code.py`; the script encodes only that file's independent `qr_destination` URL and validates it separately from `website_url`. The build copies `headshot_pic.jpg` and the fresh `qr-code.jpg` into `business_card/01_stage/output/images/`, then assembles the final HTML with `images/...` paths. Install dependencies with `business_card/.biz_card/bin/python -m pip install -r business_card/requirements.txt` when needed.
...
> Please confirm that the CARD_DATA.json information is correct before I continue. Reply `confirm` to proceed, or provide the corrections you want written to the file.

Stop and wait for the user's response. Do not continue on implied approval, silence, or a general request to build. If the user provides corrections, update only the requested fields in `CARD_DATA.json`, display the complete updated JSON again, and ask for confirmation again. Continue only after an explicit confirmation such as `confirm`, `yes, it is correct`, or equivalent.
...
| User says... | Action |
|--------------|--------|
| `build`, `create`, or `make the card` | First show `CARD_DATA.json` and obtain explicit confirmation; only then inspect assets and implement the card |
| `use this image` | Verify the supplied image path, then place it in the curved portrait frame |
| `add the QR code` | Use only the predefined `qr_destination` in `CARD_DATA.json`; it may differ from `website_url`, but never replace it with a vCard or ad hoc URL |
...
- Never generate assets or modify the card before the mandatory `CARD_DATA.json` confirmation step is complete.
