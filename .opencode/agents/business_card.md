...
- Never generate assets or modify the card before the mandatory `business_card/01_create/references/CARD_DATA.json` confirmation step is complete.
- After building, report: "The Info Pioneer business card has been successfully created and saved to `business_card/01_create/output/business_card.html`. You can now review the HTML and print it as needed."
...
| Path | Responsibility |
|------|----------------|
| `business_card/01_create/CONTEXT.md` | Authoritative design brief and exact copy |
...
| User says... | Action |
|--------------|--------|
| `build`, `create`, or `make the card` | First show `business_card/01_create/references/CARD_DATA.json` and obtain explicit confirmation; only then inspect assets and implement the card |
