...
**Behavior:**
- Reads `workspace_build/shared/references/REQUIREMENTS.md` and `RUBRIC.md` before starting.
...
- Follows `workspace_build/01_stage` for YES/NO verification questions per criterion.
- Assigns emoji ratings: ✅ Competent, ⚠️ Approaching Competence, ❌ Not Evident.
- Writes evaluation to `workspace_build/01_stage`.
...
- Reads the current report from `workspace_build/01_stage`.
- Applies fixes systematically.
- Uses websearch/webfetch to find real professional sources when needed.
- Writes the complete revised report to `workspace_build/01_stage`.
...
| Stage | Input | Output |
|-------|-------|--------|
| 1. Topic & Scope | `workspace_build/shared/references/` | `workspace_build/01_stage` |
