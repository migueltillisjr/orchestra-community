...
**Behavior:**
- Reads `ai_vid/shared/references/REQUIREMENTS.md` and `RUBRIC.md` before starting.
...
- Follows `ai_vid/01_stage` for YES/NO verification questions per criterion.
- Assigns emoji ratings: ✅ Competent, ⚠️ Approaching Competence, ❌ Not Evident.
- Writes evaluation to `ai_vid/01_stage`.
...
- Reads the current report from `ai_vid/01_stage`.
- Applies fixes systematically.
- Uses websearch/webfetch to find real professional sources when needed.
- Writes the complete revised report to `ai_vid/01_stage`.
...
| Stage | Input | Output |
|-------|-------|--------|
| 1. Topic & Scope | `ai_vid/shared/references/` | `ai_vid/01_stage` |
